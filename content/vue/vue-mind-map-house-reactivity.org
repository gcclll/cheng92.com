...
*#2* 添加 ~foo => 1~ 之后执行 effect fn forEach 迭代器进行累加操作的结果
*#3* 添加 ~bar => 2~ 结果是 4，原因是到这一步的时候 ~dummy = 1~ 的，所以再累加之
后就是 4

* TODO add collection iterators methods proxy handler
#+begin_src typescript
...
#+end_src
* TODO add readonly collection proxy handlers
