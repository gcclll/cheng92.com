...
const objAsKey = { name: 'key' }
const proxyKey = reactive(objAsKey)
const map = new Map([[proxyKey, 1]])
const observed = reactive(map)

let dummy
effect(() => {
  dummy = map.get(proxyKey)
})

console.log(`dummy = ${dummy}`)
console.log(`> proxyKey deps`)

console.log(targetMap.get(proxyKey))
#+end_src
