...
通过用例 01 大概的完成了一个比较完整的编译过程，要通过该用例应该只需要在这过程中增
加对插值的处理即可。
...
[[#compile-basecompile][baseCompile]] -> [[/vue/vue3-source-code-compiler-core-parse_ts/#parse-baseparse][baseParse]] -> [[#compile-getbasetransformpreset][getBaseTransformPreset]] 得到 transform 函数 ->
  ...
  1. baseParse -> ast
  2. [[#compile-getbasetransformpreset][getBaseTransformPreset]] -> [[#transform-transforminterpolation][transformInerpolation]]

     @@html:<font color='red'>@@TODO@@html:</font>@@  需要增加
     transformInterpolation 函数。
  ...
  3. [[#transform-transform][transform]] -> [[#transform-createtransformcontext][createTransformContext]] -> [[#transform-traversenode][traverseNode]] -> [[#transform-createrootcodegen][createRootCodegen]] -> ...
     ...
     INTERPOLATION 分支，因为 createRootCodegen 里面 root 如果只有一个孩子的情况
     下会和用例 01 一样直接赋值 ~context.codegenNode = root.children[0]~
     
  4. generate -> createCodegenContext -> genFunctionPreamble 默认是 function 模
...
1. TODO 增加 *transformInterpolation()* 函数函数
2. TODO  *traverseNode* 中增加 INTERPOLATION 分支
3. TODO  *genNode* 中增加 INTERPOLATION 分支
