...
[[http://qiniu.ii6g.com/img/yiyeshu-001.jpg]]
...
5. 调用 processCodegen 函数即 transformIf 时候执行会得到生成 codegenNode 的那个
   函数，执行它获取 tranform 函数 exitFn。
6. 手动执行 [[/vue/vue3-source-code-compiler-core-compile_ts/#transform-traversenode][traverseNode(node, ...)]] 进行递归遍历该 v-else 节点树(*因为在 4 中节点
   被删除了，因此主递归线上不会出现这个节点，因此需要手动执行一次 traverse*)
...
** DONE codegen 如何生成 if-elseif-else 分支节点 ?
