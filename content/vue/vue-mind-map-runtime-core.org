...
6. api ~watch(source, cb, option)~ 中的 source 只能是 ~reactive/ref/function/array~ 类型，
...
// 源文件：/js/vue/lib.js
const {
  rc: { nextTick, ref, watch, reactive },
  log,
} = require(process.env.BLOG_JS + "/vue/lib.js");
...
const run = async () => {
 ...
 // 监听原始类型(ref封装) deep: tru
  ...
  await nextTick();
  log.br(dummy);
  log('>> 直接监听 reactive 对象')
  const src = reactive({ count: 0 })
  watch(src, ({ count }) => { dummy = count })
  src.count++
  await nextTick()
  log.br(dummy)
};
