...
[[https://github.com/gcclll/stb-vue-next/commit/c41441a1c53b5059410c32d36e46e21e7258480c][feat(add): rc->api watch · gcclll/stb-vue-next@c41441a · GitHub]]

[[https://github.com/gcclll/stb-vue-next/commit/8c41f0f03b6f00fa88dad3389621ec089f3b6eb5][feat(add): rc->api watch -> watchEffect · gcclll/stb-vue-next@8c41f0f · GitHub]]

~watch(source, cb, options)~ 函数以下种使用方式：

1. ~sources + cb~ 方式，多个被监听对象，指定回调
...
1. 检测获取 getter 函数和 forceTrigger 标识
   ...
   #+end_src
2. cb && deep 检测是否执行深度监听，调用 traverse(baseGetter) 遍历深度监听
   ...
   #+end_src
3. scheduler job -> set scheduler 手动触发 effect
   ...
   #+end_src
4. runner = effect(getter, ...) 封装 getter 函数，待执行，这里使用了 lazy:true
   ，所以不会立即执行
...
5. record instance bound effect -> runner -> instance
   ...
   其实就是讲 effect 增加到 instance.effects[] 中。
...
测试失败，因为 ~watchEffect(fn)~ 等价于 ~watch(fn, null, options)~

进入 getter 处理步骤的 isFunction-> else
#+begin_src typescript
...
然后 else 并没有实现，所有 getter 赋值了一个空函数。
...
增加相关代码
[[https://github.com/gcclll/stb-vue-next/commit/dd4224046bc99fd60e1ef4a467d95f98ffeb912e][feat(add): rc->api watchEffect->source is fn & cb is null · gcclll/stb-vue-next@dd42240 · GitHub]]
后继续测试：
#+begin_src js
// 源文件：/js/vue/lib.js
const {
...
source 值为一个单纯的函数，正好和 [[#watch-no-cb][watch effect without cb]] 相反，会进入
isFunction>if 处理。
...
#+end_src
但这段代码貌似和 cb 没任何关系啊，怎么实现和 cb 绑定的呢？？？

#+begin_quote
在 [[/vue/vue-mind-map-house-reactivity/#r-trigger][reactivity trigger]] 中分析过， effect fn 的执行前提是没有提供 scheduler 选项，如果有呢？
#+end_quote
...
如上面的 effect 被封装成 run ，里面就有检测是不是有 scheduler ！

因此我们可以猜想 这个 cb 肯定是通过这个 options.scheduler 来调用和 source 发生关
系的。

添加代码
[[https://github.com/gcclll/stb-vue-next/commit/6766eb9066052a3608d892f3e9853627e3623043][feat(add): rc->api watch->queue scheduler for default:pre · gcclll/stb-vue-next@6766eb9 · GitHub]]
之后，测试代码考下来在测试遍：
#+begin_src js
// 源文件：/js/vue/lib.js
const {
...
#+begin_quote
Tip. 然后，[[https://github.com/gcclll/stb-vue-next/commit/6766eb9066052a3608d892f3e9853627e3623043][这里]]增加的代码全在 scheduler.ts 中，这里面都是 vue-next 里面关于 effect
trigger 任何的调度机制，需要彻底弄懂这一块，所以这里先暂停一会，先完成 [[#scheduler][scheduler
任务调度机制]] 再回顾头来继续。
#+end_quote
