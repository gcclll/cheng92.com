...
c('<textarea>some<div>text</div>and<!--comment--></textarea>',
'textarea handles comments/elements as just text', ast => ast.children)
c('<textarea>&amp;</textarea>', 'textarea handles character references')
c('<textarea><div>{{ foo }}</textarea>', 'textarea support interpolation')
c('<style>some<div>text</div>and<!--comment--></style>', 'style handles comments/elements as just a text')
c('<svg><![CDATA[some text]]></svg>', 'CDATA')
c(`<pre>  \na   <div>foo \n bar</div>   \n   c</pre>`, '<pre> tag should preserve raw whitespace')
...
c(`<div style="color: red"/>`, 'should transform into directive node，被解析成指令 :style')

l1(`vHtml.spec.ts > compiler: v-html transform`)
c(`<div v-html="test"/>`, 'should convert v-html to innerHTML')
c(`<div v-html="test">hello</div>`, 'should raise error and ignore children when v-html is present')
...
c('<input type="text" v-model="model" />', 'simple expression for input (text)')
c('<input type="radio" v-model="model" />', 'simple expression for input (radio)')
c('<input type="checkbox" v-model="model" />', 'simple expression for input (checkbox)')
c('<input :type="foo" v-model="model" />', 'simple expression for input (dynamic type)')
...
c('<my-input v-model="model" />', 'error > should allow usage on custom element')
c(`<input type="file" v-model="test"/>`,'error > should raise error if used file input element')
