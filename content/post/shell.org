...
#+begin_src sh
# 不忽略大小写
➜  test git:(master) ✗ grep -n "aaaa" ./a.org
38:aaaaaaa
39:aaaaaaa
43:aaaaaaa
# 忽略大小写
...
=--exclude pattern=, =--exclude-dir pattern= 排除 pattern 正则指定的文件/目录名，和
=--include= 先后顺序由使用时前后顺序而定
#+begin_src sh
# 使用之前
➜  test git:(master) ✗ grep -ni -e "a\{2,\}" -e "[bc]+" -e "111" ./[ab].org
./a.org:1:aaaaaaa1111
./a.org:4:ddddddd1111
./b.org:1:aaaaaaa1111
./b.org:4:ddddddd1111
...
# 使用之后(--exclude "b.org")
➜  test git:(master) ✗ grep -ni -e "a\{2,\}" -e "[bc]+" -e "111" --exclude "b.org" ./[ab].org
./a.org:1:aaaaaaa1111
./a.org:4:ddddddd1111
#+end_src
...
➜  test git:(master) ✗ cat -n ./a.org
    ...
    17	eeeeeee
    18	eeeeeee
# -m 4 所以只输出了 5~8 行的匹配结果
➜  test git:(master) ✗ grep -ni -m 4 "eeee" ./a.org
5:eeeeeee
6:eeeeeee
7:eeeeeee
8:eeeeeee
# -m 4 所以只输出了 5~9 行的匹配结果
➜  test git:(master) ✗ grep -ni -m 5 "eeee" ./a.org
5:eeeeeee
6:eeeeeee
7:eeeeeee
8:eeeeeee
