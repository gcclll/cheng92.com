...
2. git ignore 无效(如： ~.log/~)？
   ...
   $ git push
   #+end_src
3. 华为手机:iphone:现在哪里去现在谷歌商城? -> [[https://www.huaweicentral.com/download-latest-google-play-store-application-apk/][下载地址]]
4. 添加 submodule 失败 ?
...
5. ssh 登录次数过多问题(many authentication)
...
6. macos install adb
   ...
   ~$ adb devices~
7. 批量下载 bilibili 视频(you-get 命令)？
8. macos in stall java environment ?
