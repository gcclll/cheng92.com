...
5. git ignore 无效(如： ~.log/~)？
   ...
   $ git push
   #+end_src
6. 华为手机:iphone:现在哪里去现在谷歌商城? -> [[https://www.huaweicentral.com/download-latest-google-play-store-application-apk/][下载地址]]
7. 添加 submodule 失败 ?
...
8. ssh 登录次数过多问题(many authentication)
...
9. macos install adb
   ...
   ~$ adb devices~
10. 批量下载 bilibili 视频(you-get 命令)？
11. macos in stall java environment ?
